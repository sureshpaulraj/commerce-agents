...
- Dropped: explicit cache breakpoints (`cache_control`) and adaptive thinking
  ...
  and its `prompt_tokens_details.cached_tokens` is reported as `cache_read_input_tokens`,
  so a model-call log reads the same; what is lost is the control over where the prefix
  ends, not caching itself.
- Reasoning is off. A reasoning model on this endpoint refuses function tools unless
  `reasoning_effort` is `"none"`, and every turn in both roles carries tools.
...
class _Messages:
    ...
    async def stream(self, **request: Any) -> AsyncIterator[_Stream]:
        body = self._client.translate(request, stream=True)
...
class AsyncFoundryOpenAI:
    ...
    `reasoning_effort` defaults to "none" because a reasoning model on this endpoint
    refuses function tools at any other setting, and every turn in both roles carries tools.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.url = (
            ...
            or f"https://{resource}.services.ai.azure.com/openai/v1/chat/completions"
        )
