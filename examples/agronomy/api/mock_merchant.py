...
"""The agronomy example's ``MerchantBackend``: the merchant fixtures in ``data/`` overlaid
on the ``MockAgronomy`` the storefront serves, applied changes written back to that
...
from .mock_agronomy import DATA_DIR, DELIVERY_ATTRIBUTE, LOW_STOCK_ATTRIBUTE, MockAgronomy

# The one category-specific column in merchant_metrics.json, and the segment names the
# operator can ask for by.
SEGMENT_SALES_KEY = "fungicide_sales"
SEGMENT_NAMES = frozenset({"fungicide", "fungicides"})


class MockAgronomyMerchant(MerchantBackend):
    def __init__(
        self,
        storefront: MockAgronomy,
        config: MerchantAgentConfig | None = None,
        data_dir: Path = DATA_DIR,
        merchant_id: str = "heartland-agronomy",
    ) -> None:
    ...
    def home_insights(self, limit: int = 3) -> list[dict[str, Any]]:
        ...
        rows = self._daily
        if len(rows) >= 14:
            current = sum(r.get(SEGMENT_SALES_KEY, 0.0) for r in rows[-7:])
            prior = sum(r.get(SEGMENT_SALES_KEY, 0.0) for r in rows[-14:-7])
            change = change_pct(current, prior)
            if change is not None and abs(change) >= 5:
                direction = "up" if change >= 0 else "down"
                insights.append(
                    {
                        "insight_id": "segment-trend-fungicide",
                        "kind": "trend",
                        "headline": f"Fungicide sales are {direction} "
                        f"{abs(change):.0f}% week-over-week",
                        "detail": f"${current:,.0f} this week vs ${prior:,.0f} last week "
                        "for the fungicide segment.",
                        "prompt": f"Fungicide sales are {direction} {abs(change):.0f}% "
    ...
    async def query_metrics(
        ...
        def value_for(rows: list[dict[str, Any]]) -> float:
            ...
            if segment_cleaned in SEGMENT_NAMES and cleaned in {"sales", "revenue"}:
                return round(sum(r[SEGMENT_SALES_KEY] for r in rows), 2)
    ...
    ANALYSIS_SCHEMA_NOTES = (
        f"daily_metrics(date, sales, orders, traffic, {SEGMENT_SALES_KEY}) — one row per day, "
    ...
    def _analysis_connection(self) -> sqlite3.Connection:
        ...
        conn.execute(
            ...
            f"(date TEXT, sales REAL, orders INTEGER, traffic INTEGER, {SEGMENT_SALES_KEY} REAL)"
        )
        conn.executemany(
            "INSERT INTO daily_metrics VALUES (?, ?, ?, ?, ?)",
            [
                (r["date"], r["sales"], r["orders"], r["traffic"], r.get(SEGMENT_SALES_KEY))
